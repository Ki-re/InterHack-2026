...
- **Demo seed script** (`back/scripts/seed_demo.py`): fully rewritten to use `created_at`-relative timestamps. Reads each alert's `created_at` from DB (set by 0010 to May 5-7), computes `attended_at = created_at + uniform(3h, 115h)`, capped at `now - 0.5h` → all response times positive and in the past. Fractions: **55% attended (330)**, **12% dismissed (72)**, **15% touched-pending (90)**, **18% pure pending (108)**. After global seed, runs **east-zone boost**: converts all but 6 pending east alerts to attended (seed=99), giving east score ~77 (good). Idempotent. Runs automatically on every fresh `docker compose up` (via `&&` chain in docker-compose command).
  ...
  - `Landing.tsx` responsiveness: `<main>` uses `min-h-screen`; `<section>` has `xl:h-[calc(100vh-3.5rem)] xl:overflow-hidden` for viewport-fit at xl+. **CSS `zoom` scaling**: at xl, a `useEffect` calculates `zoom = min(1, max(0.6, (viewportHeight - 56) / 900))` and applies it as inline `style={{ zoom, height }}` on the section. This proportionally scales ALL content (text, padding, images, gaps) on shorter screens instead of clipping. The height is compensated as `calc((100vh - 3.5rem) / zoom)` so the zoomed section still fills the viewport. All inner `xl:overflow-hidden` removed (no longer needed). `xl:grow xl:min-h-0` kept on panels/children for internal flex distribution. Text scales up at 2xl breakpoint (≥1536px) via responsive classes.
