...
- **Startup robustness** (`back/app/main.py`): `run_daily_notifications()` in lifespan is now wrapped in `try/except` — a notification job failure (e.g., SQLite WAL race on first container start) logs a warning but no longer crashes the application startup.
